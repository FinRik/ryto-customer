...
class HandlingOptionsCard extends StatelessWidget {
  const HandlingOptionsCard({
    super.key,
  ...
  void _toggleOption(HandlingOption option, bool isSelected) {
    final updatedList = List<String>.from(selectedOptions);
    final targetString = _enumToString(option);

    if (isSelected) {
      if (!updatedList.contains(targetString)) {
        updatedList.add(targetString);
      }
    } else {
      updatedList.remove(targetString);
    }
  ...
  @override
  Widget build(BuildContext context) {
    final hasFragile = selectedOptions.contains(_enumToString(HandlingOption.fragile));
    final hasSignature = selectedOptions.contains(_enumToString(HandlingOption.requireSignature));
      ...
      children: [
        CustomTileWidget(
          ...
          trailing: Checkbox(
            ...
            onChanged: (bool? checked) => _toggleOption(HandlingOption.fragile, checked ?? false),
          ),
        ),
        ...
        CustomTileWidget(
          ...
          trailing: Checkbox(
            value: hasSignature,
            activeColor: const Color(0xff002252),
            onChanged: (bool? checked) => _toggleOption(HandlingOption.requireSignature, checked ?? false),
          ),
        ),
      ],
    );
