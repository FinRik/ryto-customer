...
class TripSetupBloc extends Bloc<TripSetupEvent, TripSetupState> {
  ...
  TripSetupBloc(this.repo) : super(TripSetupState(request: BookingRequest())) {
    on<UpdateTripProgress>(_onUpdateProgress);
    on<FetchRoutesRequested>(_onFetchPopularRoutes);
    on<FetchAvailableTripsRequested>(_onFetchAvailableTrips);
    on<FetchTripCostRequested>(_onFetchCost);
    on<ScheduleTripRequested>(_onScheduleTrip);
  }

  void _onUpdateProgress(
    UpdateTripProgress event,
  ...
  Future<void> _onFetchAvailableTrips(
    FetchAvailableTripsRequested event,
    Emitter<TripSetupState> emit,
  ) async {
  ...
  Future<void> _onFetchCost(
    FetchTripCostRequested event,
    Emitter<TripSetupState> emit,
  ) async {
    emit(state.copyWith(setupStatus: TripSetupStatus.loading));
    try {
      final res = await repo.fetchBookingCost(state.request);
      if (res != null) {
        emit(
          state.copyWith(
            setupStatus: TripSetupStatus.success,
            costSummary: res,
          ),
        );
      } else {
        emit(
          state.copyWith(
            setupStatus: TripSetupStatus.failure,
